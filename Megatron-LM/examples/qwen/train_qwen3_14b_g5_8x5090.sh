...
if [[ "$PRESET" == "vpp_morphable_budgeted" ]]; then
  if [[ "$RUN_TARGET" != "single_g5" ]]; then
    echo "Invalid preset: vpp_morphable_budgeted currently requires RUN_TARGET=single_g5"
    exit 1
  fi
  MODEL_TRACK="dense"
  TP_SIZE=1
  ...
  VPP_SIZE=2
  PIPELINE_LAYOUT=${PIPELINE_LAYOUT:-$VPP_NEIGHBOR_LAYOUT}
  SCHEDULE_TEMPLATE=${SCHEDULE_TEMPLATE:-pp4_middle_relief}
  SCHEDULE_GROUP_SIZE=${SCHEDULE_GROUP_SIZE:-8}
  DISPATCH_ORDER=${DISPATCH_ORDER:-structure_aware_critical_first}
  SCHEDULE_WARMUP_POLICY=${SCHEDULE_WARMUP_POLICY:-balanced_fill}
  SCHEDULE_COOLDOWN_POLICY=${SCHEDULE_COOLDOWN_POLICY:-opt_prioritized}
  SCHEDULE_WARMUP_CHECKPOINT_POLICY=${SCHEDULE_WARMUP_CHECKPOINT_POLICY:-full}
  SCHEDULE_STEADY_CHECKPOINT_POLICY=${SCHEDULE_STEADY_CHECKPOINT_POLICY:-default}
  SCHEDULE_WARMUP_COMBINED_POLICY=${SCHEDULE_WARMUP_COMBINED_POLICY:-serial}
  SCHEDULE_STEADY_COMBINED_POLICY=${SCHEDULE_STEADY_COMBINED_POLICY:-combined}
  SCHEDULE_COOLDOWN_P2P_POLICY=${SCHEDULE_COOLDOWN_P2P_POLICY:-serial}
  SCHEDULE_COOLDOWN_COMBINED_POLICY=${SCHEDULE_COOLDOWN_COMBINED_POLICY:-serial}
  SCHEDULE_FLUSH_ORDER_POLICY=${SCHEDULE_FLUSH_ORDER_POLICY:-reverse_last_group}
  RECOMPUTE_GRANULARITY=${RECOMPUTE_GRANULARITY:-selective}
  ENABLE_RECOMPUTE_ACTIVATIONS=${ENABLE_RECOMPUTE_ACTIVATIONS:-1}
  RECOMPUTE_MODULES=${RECOMPUTE_MODULES:-core_attn,mlp}
  if [[ "${ENABLE_FINE_GRAINED_ACTIVATION_OFFLOADING:-0}" == "1" ]]; then
    OFFLOAD_MODULES=${OFFLOAD_MODULES:-core_attn,attn_proj}
  fi
fi
