...
class ParallelDims:
    ...
    def _validate(self):
        ...
        if self.rank_order is not None:
            if len(self.rank_order) != self.world_size:
                raise ValueError(
                    f"rank_order must have len={self.world_size}; got {len(self.rank_order)}"
                )
            if sorted(int(x) for x in self.rank_order) != list(range(self.world_size)):
                raise ValueError(
                    "rank_order must be a permutation of [0, world_size)"
                )
    ...
    def build_mesh(self) -> DeviceMesh:
        ...
        if self.rank_order is None:
            ...
            rank_tensor = torch.tensor(
                [int(x) for x in self.rank_order],
                dtype=torch.int,
            )
            ...
            logger.info(f"Using custom rank_order for world mesh: {self.rank_order}")
