...
class ParallelismConfig:
    ...
    fsdp_parallelism_conditioned_policy: Literal["uniform", "module_groups"] = "uniform"
    """
    Experimental FSDP policy mode.

    - "uniform": use TorchTitan's existing whole-block / whole-model FSDP wrapping.
    - "module_groups": use Qwen3-specific module-group-aware wrapping that can choose
      different reshard scopes for attention, MLP, and embedding/head groups.
    """

    fsdp_attention_scope: Literal["auto", "global", "node", "keep"] = "auto"
    """Experimental reshard scope for attention groups under module-group FSDP."""
    ...
    """
    Experimental node-local reshard size used when a module-group scope resolves to "node".
    Set to the intended intra-node shard world size. Values <= 0 disable node-local scope.
    """
    ...
    pipeline_parallel_stage_to_node: list[str] | None = None
    """
    Optional per-stage node labels, primarily for heterogeneous placement planning and logging.
    This does not by itself change rank placement; it is consumed by experimental adapters and
    validation logic.
    """

    rank_order: list[int] | None = None
    """
    Optional explicit global-rank ordering for world mesh construction.
    When provided, the world mesh is built using this permutation instead of natural rank order.
    This enables experimental heterogeneous PP placement without changing launcher rank IDs.
    """
    ...
    pipeline_parallel_vpp_per_rank: list[int] | None = None
    """
    Optional experimental VPP counts per physical PP rank.
    TorchTitan's current runtime still assumes a uniform number of local stages per rank for execution.
    This field is therefore used for validation/planning/debugging first, and only supports uniform
    values in the current runtime path.
    """
...
class DebugConfig:
    ...
    """Enable stage-level pipeline tracing for stage construction and chunk execution."""
